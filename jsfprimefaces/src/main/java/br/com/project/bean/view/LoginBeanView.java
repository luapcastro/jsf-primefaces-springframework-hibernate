...
public class LoginBeanView extends BeanManagedViewAbstract {
	...
	public void invalidarSessionMethod(HttpServletRequest httpServletRequest) throws Exception {
		...
		if (userLogadoSessao == null || userLogadoSessao.trim().isEmpty()) {
	...
	public void invalidar(ActionEvent actionEvent) throws Exception {
		...
		if (srvLogin.autentico(getUsername(), getPassword())) {
			...
			loggedIn = false;
			message = new FacesMessage(FacesMessage.SEVERITY_WARN, "Acesso negado", "Login e/ou senha inválidos");
		}
		...
		context.addCallbackParam("loggedIn", context);
	}
